...
with open('test_data_3.csv', 'w', encoding="UTF8") as file:
...
#------------------------------------LOOP to buiild db with info from google api-----------------------------------------
    ...
    for page in range(pages):
        ...
        url = f"https://www.googleapis.com/customsearch/v1?key={API_KEY}&tbm={type}&cx={SEARCH_ENGINE_ID}&q={query}&start={start}"
        ...
        for i, search_item in enumerate(search_items, start):
            ...
            writer.writerow([title, page_soup.find_all('^p'), link])
